...
export class MyContract extends Contract {
    ...
    // below - txn function can be called by client (JSON), internal demo setup function (JS object), or
    // as a VSCode submit txn (ie param values supplied only) - function will handle all 3 scenarios

    /**
     * depart an animal
     *
     * @param {Context} ctx the transaction context
     * @param {String} animalDetails species
     */

    public async register(ctx: Context, animalDetails: string): Promise<any> {

        let animal: MyAnimal;

        if (Utils.checkJSON(animalDetails)) {
            ...
            const [species, id, dob, description, owner, location, field, status, productionType, isVaccinated]: string[] = animalDetails.toString().split(' ');
            // parameters passed eg.
            // ["SHEEPGOAT 000011 24/07/2019 BOVIS_ARIES FARMER.JOHN AVONDALE.LOC1 ARRIVALF1 IN_FIELD WOOL false"]
            // ["SHEEPGOAT 000012 25/07/2019 BOVIS_ARIES FARMER.JOHN AVONDALE.LOC2 ARRIVALF1 IN_FIELD WOOL false"]
            // ["SHEEPGOAT 000013 26/07/2019 BOVIS_ARIES FARMER.JOHN AVONDALE.LOC2 ARRIVALF1 IN_FIELD RESEARCH false"]
            // becomes

            // eg. species = 'SHEEPGOAT'; id = '0009'; location = 'AVONDALE.LOC1';field = 'ARRIVALF1'; status = 'IN_FIELD' ; productionType = 'WOOL' ; isVaccinated = false;

            animal = MyAnimal.createInstance(species as Species, id, dob, description, owner, location, field, status as Status, productionType as ProductionType, JSON.parse(isVaccinated));

            // let classname = animal.getClass();   // fyi generalClass (see model file) methods can be called here
    ...
    */
    public async setupdemo(ctx: Context, prefix: string): Promise<any> {
        ...
        // JS Objects or just raw parameter string as shown in 'register' main function eg
        // Parameters only: see 'register' function eg
        // "SHEEPGOAT 000011 24/07/2019 BOVIS_ARIES FARMER.JOHN AVONDALE.LOC1 ARRIVALF1 IN_FIELD WOOL false"
    ...
    /**
     * register an animal: takes a class object instance as a param target statelist is AnimalClass
     *
     * @param {Context} ctx the transaction context
